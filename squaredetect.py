...
def redo(im_with_keypoints):
	...
	for j in range(len(removebar)):
		print(removebar[j])
		tree = spatial.KDTree(BarCoordinates)
		dist, index= tree.query(removebar[j])
		indexes = []
		BarCoordinates.remove(index)
		#BarCoordinatesend[index][0] = 0
		#BarCoordinatesend[index][1] = 0
		#BarCoordinatesend[index][2] = 0
		...
		for light in keypoints_light:
			...
			#print(deg)
				...
				if line_length_lower < ((xd-xl)**2 + (yd-yl)**2)**0.5 < line_length_upper and ((deg<degree_change) or ((90-degree_change)<deg<(90+degree_change)) or ((180-degree_change)<deg<(180+degree_change)) or ((270-degree_change)<deg<(270+degree_change)) or (deg>(360-degree_change))):
					...
					BarCoordinates.append([xa,ya])	
					BarCoordinatesend.append([cl,cd,deg])
					IslandProperties.append([xa,ya,mx,my])
...
xpos = np.zeros((2*dimension+1, 2*dimension+1))
ypos = np.zeros((2*dimension+1, 2*dimension+1))
mxgrid = np.zeros((2*dimension+1, 2*dimension+1))
mygrid = np.zeros((2*dimension+1, 2*dimension+1))
...
for y in range(2*dimension+1):
	ypos[y] = (2*dimension - y)

for bar in b:
	print(bar[0], bar[1])
	mxgrid[(bar[1]),(bar[0])]=bar[2]
	mygrid[(bar[1]),(bar[0])]=bar[3]
	if bar[0]%2 == 0 and bar[1]%2==0:
		mxgrid[(bar[1]),(bar[0])]=0
		mygrid[(bar[1]),(bar[0])]=0
...
q= ax.quiver(xpos, ypos, mxgrid, mygrid, np.arctan2(mygrid, mxgrid), pivot='mid')
plt.show()
